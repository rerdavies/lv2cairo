...
LvtkScrollBarElement::LvtkScrollBarElement()
{
    OrientationProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    WindowSizeProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    DocumentSizeProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    ScrollOffsetProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    ThumbHoverColorsProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    TrackHoverColorsProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    AnimationValueProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);
    FlubberFactorProperty.SetElement(this, LvtkBindingFlags::InvalidateOnChanged);

}

bool LvtkScrollBarElement::WillDraw() const
{
    return true;
}

static LvtkRectangle BlendRect(double value, const LvtkRectangle &r1, const LvtkRectangle &r2)
{
    double v0 = 1 - value;
...
LvtkRectangle LvtkScrollBarElement::GetTrackRectangle() const
{
    if (Orientation() == LvtkScrollBarOrientation::Vertical)
    {
...
LvtkRectangle LvtkScrollBarElement::GetThumbHitRect() const
{
    if (Orientation() == LvtkScrollBarOrientation::Vertical)
    {
        // ignore the animation. hit test is on AnimationValue() == 1.0 rendering.
        LvtkRectangle clientRectangle{this->ClientSize()};
        const LvtkRectangle &paddingRectangle = ClientPaddingRectangle();
        double thumbRadius = clientRectangle.Width() / 2;
...
void LvtkScrollBarElement::OnDraw(LvtkDrawingContext &dc)
{
    LvtkRectangle clientRectangle{this->ClientSize()};
    LvtkRectangle paddingRectangle = ClientPaddingRectangle();
    ...
    if (Orientation() == LvtkScrollBarOrientation::Vertical)
    {
        ...
        LvtkRectangle trackRect = clientRectangle;

        LvtkStyle *collapsedStyle = GetCollapsedStyle();

        LvtkRectangle collapsedTrackRect = LvtkRectangle(
        ...
        LvtkRoundCorners roundCorners = Style().RoundCorners().PixelValue();

        LvtkColor trackColor = LvtkColor::Blend(
            AnimationValue(),
            TrackHoverColors().GetColor(LvtkHoverState::Empty),
            TrackHoverColors().GetColor(HoverState()));
        dc.round_corner_rectangle(trackRect, roundCorners);
        dc.set_source(trackColor);
        dc.fill();

        LvtkThickness collapsedPadding = collapsedStyle->Padding().PixelValue();
        LvtkRectangle collapsedThumbTrackRect = LvtkRectangle(
        ...
        LvtkRectangle thumbTrackRect = BlendRect(AnimationValue(), collapsedThumbTrackRect, clientRectangle);
        ...
        double thumbTop = scrollTop / DocumentSize() * (maxY - minY) + minY;
        double thumbBottom = scrollBottom / DocumentSize() * (maxY - minY) + minY;
        LvtkRectangle thumbRect{
            thumbTrackRect.Left(),
            thumbTop - thumbRadius,
            thumbTrackRect.Width(),
            thumbBottom - thumbTop + 2 * thumbRadius};

        LvtkColor thumbColor = LvtkColor::Blend(
            AnimationValue(),
            ThumbHoverColors().GetColor(LvtkHoverState::Empty),
        ...
        dc.round_corner_rectangle(
            thumbRect,
            LvtkRoundCorners{thumbRadius, thumbRadius, thumbRadius, thumbRadius});
...
bool LvtkScrollBarElement::OnMouseOver(LvtkMouseOverEventArgs &event)
{
    StartAnimation(1.0);
    return true;
}
bool LvtkScrollBarElement::OnMouseOut(LvtkMouseOverEventArgs &event)
{
...
double LvtkScrollBarElement::ComputeFlubberFactor(double overscroll)
{
    if (overscroll < 0)
    {
...
void LvtkScrollBarElement::AnimationTick(const animation_clock_time_point_t &now)
{
...
void LvtkScrollBarElement::StartAnimation(double targetValue)
{
...
void LvtkScrollBarElement::StopAnimation()
{
    if (animationHandle)
    {
...
void LvtkScrollBarElement::OnMount()
{
    this->ClearClasses();
    super::OnMount();
    auto &theme = Theme();
...
void LvtkScrollBarElement::OnUnmount()
{
    if (animationHandle)
    {
        Window()->CancelAnimationCallback(animationHandle);
...
void LvtkScrollBarElement::OnHoverStateChanged(LvtkHoverState hoverState)
{
    super::OnHoverStateChanged(hoverState);
    Invalidate();
}

LvtkStyle *LvtkScrollBarElement::GetCollapsedStyle() const
{
    if (CollapsedStyle())
    {
        return CollapsedStyle().get();
    }
    if (Orientation() == LvtkScrollBarOrientation::Horizontal)
    {
...
bool LvtkScrollBarElement::OnMouseDown(LvtkMouseEventArgs &event)
{
    ...
    // LvtkRectangle mouseHitRect = GetThumbHitRect();
...
bool LvtkScrollBarElement::OnMouseUp(LvtkMouseEventArgs &event)
{
    ...
    if (this->Capture() == this)
    {
        if (!isMouseDragging)
        {
            ...
            LvtkRectangle trackRectangle = this->GetTrackRectangle();
            if (this->Orientation() == LvtkScrollBarOrientation::Vertical)
            {
...
bool LvtkScrollBarElement::OnMouseMove(LvtkMouseEventArgs &event)
{
    if (LvtkPoint::Distance(event.screenPoint, mouseDownPoint) >= 2.4)
    {
        isMouseDragging = true;
    }
    if (isMouseDragging)
    {
        LvtkRectangle trackRectangle = GetTrackRectangle();
        double scrollOffset;
        if (Orientation() == LvtkScrollBarOrientation::Vertical)
        {
